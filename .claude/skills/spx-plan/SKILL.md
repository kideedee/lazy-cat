...
### On Entry: Handle Uncommitted Changes

When `/spx-plan` is invoked, check for uncommitted changes:
...
```
⚠️ Có uncommitted changes:
[list of changed files]

Bạn muốn:
A. Commit changes trước (tôi sẽ hỏi commit message)
B. Stash changes (git stash push -m "WIP before spx-plan")
C. ★ Tiếp tục trên branch hiện tại (không tạo branch mới)
D. Khác: ___
```

If user chooses A → prompt for commit message, commit, then continue.
If user chooses B → stash with descriptive message, then continue.
If user chooses C → skip branch creation at end of discovery, work on current branch.

### On Exit: Create Feature Branch

When discovery is complete and user is ready to implement (choosing `/spx-apply` or `/spx-ff` from "Ready for Plan" options), create a feature branch:

1. **Derive branch name** from discussion context (feature description)
2. **Suggest branch name** to user for confirmation:

```
📌 Tạo branch mới từ master:
Đề xuất: feat/<derived-description>/sonvq
...
3. **Create branch** if confirmed:

```bash
git fetch origin master
git checkout -b feat/<description>/sonvq origin/master
```

4. **Continue to implementation** (`/spx-apply` or `/spx-ff`)
...
- Examples:
  - `feat/add-dark-mode/sonvq`
  - `fix/mermaid-rendering/sonvq`
  - `chore/update-deps/sonvq`
